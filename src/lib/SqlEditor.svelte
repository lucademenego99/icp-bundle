...
<script lang="ts">
    ...
    // Create an HTML table from the output of type Table
    ...
    onMount(async () => {
        ...
        webworker.onerror = (e) => {
            console.warn("[ERROR] Worker error: ", e);
        };
        ...
        webworker.onmessage = (message) => {
            ...
            if (message.data.error) {
                ...
                outputError = false;
                let text = "";
                // Check if message.data.result is an array
                ...
                if (
                    ...
                    outputElement.innerHTML = tables.join("<br>");
                } else {
...
<div bind:this={rootElement} id="code-container">
    ...
    <div
        ...
        style='{type == "vertical"
            ...
            : "right: 0; top: -38px;"}' 
    >
        <button id="run-button" on:click={runCode}>
