...
<script lang="ts">
    ...
    async function downloadSnippet() {
        const name = `${id != "" ? id : "code-snippet"}.${languageToExtension[language]}`;
                ...
                types: [
                    {
                        ...
                        accept: {
                            [languageToMimeType[language]]: [`.${languageToExtension[language]}`],
                        },
                    },
                ],
            });
...
<button
    ...
    style="position: absolute; right: {type == 'vertical'
        ...
        : `min(2vw, 4vh)`}; top: min(0.5vw, 1vh); border: 0px; border-radius: .4em; display: flex; justify-content: center; align-items: center; z-index: 99; background-color: transparent; cursor: pointer;"
>
    <svg
